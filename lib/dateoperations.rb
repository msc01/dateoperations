...
# All date operations are implemented as class-level methods.
class DateOperations
  ...
  # Returns an array with dates for the "week days" (Monday - Friday) between a given start and end date.
  def self.week_days_between(start_date, end_date)
    week_day_nbrs = (1..5)
    (start_date..end_date).select { |w| week_day_nbrs.include?(w.wday) }
  end

  # returns the number of "week days" (Monday - Friday) between a given start and end date.
  def self.number_of_week_days_between(start_date, end_date)
    week_days = week_days_between(start_date, end_date)
    week_days.length
  end

  # Returns an array with dates for the "business days" = "week days (Monday - Friday) without holidays" between a given start and end date.
  def self.business_days_between(start_date, end_date)
    week_days = week_days_between(start_date, end_date)
    week_days.reject { |b| holiday?(b) }
  end

  # Returns the number of "business days" = "week days (Monday - Friday) without holidays" between a given start and end date.
  ...
  # Checks whether a given date is a weekend (Saturday, Sunday = true) or not (= false).
